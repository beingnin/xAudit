...
BEGIN
	...
	BEGIN
		...
		BEGIN
			INSERT INTO [xAudit].[DDL_CHANGES]
			([SOURCE_SCHEMA],
			 [SOURCE_TABLE],
			 [DDL_COMMAND],
			 [MERGED],
			...
			SELECT OBJECT_SCHEMA_NAME(INSERTED.[SOURCE_OBJECT_ID]),
			OBJECT_NAME(INSERTED.[SOURCE_OBJECT_ID]),
			INSERTED.DDL_COMMAND,
			0, 
			INSERTED.[DDL_TIME],
			[xAudit].[GET_VERSION](OBJECT_SCHEMA_NAME(INSERTED.[SOURCE_OBJECT_ID]),OBJECT_NAME(INSERTED.[SOURCE_OBJECT_ID])) FROM INSERTED
		END');
...
CREATE PROCEDURE [xAudit].[MERGE]( @SOURCE_SCHEMA SYSNAME, @SOURCE_TABLE SYSNAME, @VERSION SMALLINT)
AS BEGIN
	SET @VERSION=@VERSION-1; --	TAKING DDLS FROM PREVIOUS VERSION
	DECLARE @T AS TABLE (SL SMALLINT IDENTITY(1,1), COMMAND NVARCHAR(MAX));

	INSERT INTO @T SELECT DDL_COMMAND FROM [xAudit].[DDL_CHANGES]
	WHERE SOURCE_SCHEMA = @SOURCE_SCHEMA AND SOURCE_TABLE = @SOURCE_TABLE AND [MERGED]<> 1 AND [VERSION]=@VERSION ORDER BY CHANGE_DATE 
	...
	UPDATE [xAudit].[DDL_CHANGES] SET [MERGED] = 1 WHERE SOURCE_SCHEMA = @SOURCE_SCHEMA AND SOURCE_TABLE = @SOURCE_TABLE AND  [VERSION]=@VERSION;
...
AS BEGIN
	...
	DECLARE @CHANGES TABLE(SL INT IDENTITY(1,1),SCOL SYSNAME,ACOL SYSNAME,STYPE SYSNAME, ATYPE SYSNAME,QUERY NVARCHAR(MAX))
...
BEGIN
	...
	BEGIN
		...
		ELSE BEGIN --TABLE EXISTS
			...
			EXEC [xAudit].[MERGE] @SOURCE_SCHEMA, @SOURCE_TABLE, @FORCE
		END
