...
(
	...
	DIFFERENCES AS
	(
		SELECT * FROM TSOURCE EXCEPT SELECT * FROM COMMON
		UNION ALL
		SELECT * FROM TCDC EXCEPT SELECT * FROM COMMON
	)
	SELECT T.*,CASE WHEN D.CAPTURE_INSTANCE IS NULL THEN 1 ELSE 0 END AS SCHEMACHANGED FROM TCDC T LEFT JOIN DIFFERENCES D ON T.CAPTURE_INSTANCE=D.CAPTURE_INSTANCE
)
